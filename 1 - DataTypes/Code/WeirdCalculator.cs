...
namespace DataTypes.Code
{
	public static class WeirdCalculator
	{
		...
		{
            var result = (long)firstValue+secondValue;
            return result;
		...
		public static decimal ComplexAdd(char firstValue, string secondValue)
		{
			 ...
			 */
            var result = decimal.Parse(char.ToString(firstValue))+decimal.Parse(secondValue); //this seems messy but since ToDecimal(Char) is not supported I couldn't think of any other way
            return result;
			throw new NotImplementedException();
		}
